...
export class AdminHierarchyService {
  ...
  async canAccessZone(adminId: string, zoneId: string): Promise<boolean> {
    ...
    switch (admin.role) {
      ...
      case Role.STATE_ADMIN:
        ...
        return (
          branch && admin.state._id.toString() === branch.stateId.toString()
        );
      case Role.BRANCH_ADMIN:
  ...
  async getAccessibleBranches(
    ...
    switch (admin.role) {
      case Role.SUPER_ADMIN:
        const query = stateId
          ? { stateId: new Types.ObjectId(stateId), isActive: true }
          : { isActive: true };
    ...
    switch (admin.role) {
      case Role.SUPER_ADMIN:
        const query = branchId
          ? { branchId: new Types.ObjectId(branchId), isActive: true }
          : { isActive: true };
   ...
   */
  private async getBranchIdsInState(
    stateId: string,
  ...
  async calculateMarketerPerformanceRating(
    ...
    const totalInvited = await this.guestModel.countDocuments({
    ...
    if (checkInRate >= 0.9)
      ...
      rating = 4; // 80-89% = 4 stars
    else if (checkInRate >= 0.7)
      rating = 3; // 70-79% = 3 stars
    else if (checkInRate >= 0.6)
      rating = 2; // 60-69% = 2 stars
  ...
  async getMarketersPerformanceSummary(adminId: string): Promise<any[]> {
    ...
    const marketers = await this.userModel
      ...
      .populate('zone', 'name')
      .select(
        'name email performanceRating totalInvitedAttendees totalCheckedInAttendees state branch zone',
      )
    ...
    return marketers.map((marketer) => ({
      ...
      checkInRate:
        marketer.totalInvitedGuests > 0
          ? Math.round(
              (marketer.totalCheckedInGuests / marketer.totalInvitedGuests) *
                100,
            ) / 100
          : 0,
      location: {
  ...
  async getPendingZoneAdminsByBranch(
    ...
    const allZonalAdmins = await this.userModel
      .find({ role: Role.ZONAL_ADMIN })
      .exec();
    const matchingAdmins = allZonalAdmins.filter(
      (za) =>
        za.branch &&
        za.branch.toString() === branchId &&
  ...
  async approveZoneAdmin(
    ...
    if (branchAdmin.role !== Role.BRANCH_ADMIN) {
      throw new ForbiddenException(
        'Only branch admins can approve zone admins',
      );
    }
    ...
    if (zoneAdmin.branch.toString() !== branchId) {
      throw new ForbiddenException(
        'You can only approve zone admins in your branch',
      );
    }
  ...
  async rejectZoneAdmin(
    ...
    if (zoneAdmin.branch.toString() !== branchId) {
      throw new ForbiddenException(
        'You can only reject zone admins in your branch',
      );
    }
  ...
  async replaceAdmin(
    ...
    const newAdminHasAssignment =
      await this.checkAdminJurisdictionAssignment(newAdmin);
  ...
  async transferJurisdiction(
    ...
    // Validate hierarchy permissions
    if (
      !this.canManageAdmin(requestingAdmin.role, fromAdmin.role) ||
    ...
    const toAdminHasAssignment =
      await this.checkAdminJurisdictionAssignment(toAdmin);
    ...
    switch (transferDto.adminRole) {
      case Role.STATE_ADMIN:
        ...
        if (fromAdmin.state?.toString() !== transferDto.stateId) {
          throw new BadRequestException(
            'Source admin does not manage this state',
          );
        }
      ...
      case Role.BRANCH_ADMIN:
        ...
        if (fromAdmin.branch?.toString() !== transferDto.branchId) {
          throw new BadRequestException(
            'Source admin does not manage this branch',
          );
        }
      ...
      case Role.ZONAL_ADMIN:
        ...
        if (fromAdmin.zone?.toString() !== transferDto.zoneId) {
          throw new BadRequestException(
            'Source admin does not manage this zone',
          );
        }
        toAdmin.zone = new Types.ObjectId(transferDto.zoneId);
        toAdmin.branch = new Types.ObjectId(
          (zone.branchId as any)._id || zone.branchId,
        );
        toAdmin.state = new Types.ObjectId(
          (zone.branchId as any).stateId._id || (zone.branchId as any).stateId,
        );
        ...
        transferredJurisdiction.branchId = (
          zone.branchId as any
        )._id.toString();
        transferredJurisdiction.branchName = (zone.branchId as any).name;
        transferredJurisdiction.stateId = (
          zone.branchId as any
        ).stateId._id.toString();
   ...
   */
  private async checkAdminJurisdictionAssignment(
    admin: UserDocument,
  ): Promise<boolean> {
    switch (admin.role) {
      case Role.STATE_ADMIN:
